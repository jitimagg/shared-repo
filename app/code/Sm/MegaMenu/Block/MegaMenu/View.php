...
class View extends Template
{
    ...
    {
        return [
            'BLOCK_TPL_SM_MEGAMENU',
            $this->_storeManager->getStore()->getCode(),
            $this->_storeManager->getStore()->getId(),
            $this->_storeManager->getStore()->getCurrentCurrencyCode(),
			$this->_storeManager->getStore()->getCurrentUrl(false),
            $this->getNameInLayout(),
            $this->getTemplateFile(),
            'base_url' => $this->getBaseUrl(),
            'template' => $this->getTemplate(),
            $this->_getConfig('group_id')
        ];
    }
	...
	public function getLinkOfType($item){
        ...
        elseif($item['type'] == self::PAGE_MODULE){
            ...
            return $storeManager->getStore()->getBaseUrl(\Magento\Framework\UrlInterface::URL_TYPE_WEB).$item['data_type'];
        }
		else
			return '#';
	}
	...
	public function getItemHtml($item, $isFirstColumn ='', $idActive = ''){
        ...
        if($item['type'] == '4' && $item['data_type'] && $item['show_title_category'] == "1"){
            $idCate   = explode('/', $item['data_type']);
            $category = $this->_objectManager->create('Magento\Catalog\Model\Category')->load($idCate[1]);
...
//								return $html;
	...
	public function getCategory($item, $itemId){
		...
		if ($item['show_title_category'] == self::STATUS_ENABLED)
		{
			...
			//$output .= '<h3 class="'.$aClassName.' '.$activedClassName.' title-cat"><a href="'.$catParentUrl.'">'.__($name_cat_parent).'</a></h3>';
			$output .= '</div>';
		}
		if ($id_all_cat)
		{
