 ...
 *
 */
namespace Clarion\CustomerAttribute\Block\Adminhtml\Attribute;
...
class Edit extends \Magento\Backend\Block\Widget\Form\Container
{
     ...
     * @var string
     */
    protected $_blockGroup = 'Clarion_CustomerAttribute';
    ...
    protected function _construct()
    {
        ...
        if (!$entityAttribute || !$entityAttribute->getIsUserDefined()
                || !$this->_isAllowedAction('Clarion_CustomerAttribute::attribute_delete')) {
            $this->buttonList->remove('delete');
        } else {
            $this->buttonList->update('delete', 'label', __('Delete Attribute'));
        }

        if ((!$this->_isAllowedAction('Clarion_CustomerAttribute::save')) ||
