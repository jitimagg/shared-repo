...
class ListingTabs extends \Magento\Catalog\Block\Product\AbstractProduct
{
    ...
    {
        $defaults = [];
        $_cfg_xml = $this->_scopeConfig->getValue('listingtabs', \Magento\Store\Model\ScopeInterface::SCOPE_STORE,
            $this->_storeCode);
        if (empty($_cfg_xml)) {
            return;
        }
        ...
        foreach ($groups as $group) {
            $_cfgs = $this->_scopeConfig->getValue('listingtabs/' . $group . '',
                \Magento\Store\Model\ScopeInterface::SCOPE_STORE, $this->_storeCode);
    ...
    protected function _toHtml()
    {
        ...
        if ($this->_isAjax()) {
            if ($this->_getConfig("template_style") == "default_items_left") {
                $template_file = "Sm_ListingTabs::default_items_left.phtml";
            } else {
                if ($this->_getConfig("template_style") == "default_items_center") {
                    $template_file = "Sm_ListingTabs::default_items_center.phtml";
                } else {
                    if ($this->_getConfig("template_style") == "default_items_cat") {
                        $template_file = "Sm_ListingTabs::default_items_v30.phtml";
                    } else {
                ...
                if (!empty($category_id)) {
                    ...
                    if (!empty($all_childrens)) {
                        ...
                        foreach ($all_childrens as $key => $children) {
							...
							$list[$key]['cat_img'] =  $object_manager->getImageUrl();
                            if ($flag) {
                                $list[$key]['sel']           = 'active';
    ...
    private function _getOrderedQty(&$collection)
    {
        ...
        $select     = $connection
            ->select()
            ->from($connection->getTableName($this->_resource->getTableName('sales_bestsellers_aggregated_monthly')),
                array('product_id', 'ordered_qty' => 'SUM(`qty_ordered`)'))
    ...
    private function _getViewedCount(&$collection)
    {
        ...
        $select     = $connection
            ->select()
            ->from($connection->getTableName($this->_resource->getTableName('report_event')),
                ['*', 'num_view_counts' => 'COUNT(`event_id`)'])
